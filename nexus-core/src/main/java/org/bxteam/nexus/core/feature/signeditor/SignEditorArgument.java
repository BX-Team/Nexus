...
@LiteArgument(type = int.class, name = SignEditorArgument.KEY)
public class SignEditorArgument extends MultificationLiteArgument<Integer> {
    private static final List<Integer> suggestions = List.of(1, 2, 3, 4);
    static final String KEY = "sign-editor-line";

    @Inject
    public SignEditorArgument(TranslationProvider translationProvider, PluginConfigurationProvider configurationProvider) {
    ...
    public ParseResult<Integer> parse(Invocation<CommandSender> invocation, String argument, Translation translation) {
        try {
            ...
            if (value < 1 || value > 4) {
                return ParseResult.failure(translation.signEditor().invalidIndex());
            }
