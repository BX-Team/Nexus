...
public class HealCommand extends CommandBase implements CommandExecutor, TabCompleter {
    ...
    protected void execute(CommandSender sender, String label, String[] args) {
        if (args.length == 0) {
            ...
            sender.sendMessage(TextUtils.applyColor(Nexus.getInstance().localeReader.getPrefix() + Nexus.getInstance().localeReader.getString("heal-other-error").replace("%t", target.getName())));
        } else {
            sender.sendMessage(TextUtils.applyColor(Nexus.getInstance().localeReader.getPrefix() + Nexus.getInstance().getConfigString("heal-usage")));
        }
    }
