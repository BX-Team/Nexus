...
public class InvSeeCommand extends CommandBase implements CommandExecutor, TabCompleter {
    public InvSeeCommand() {
        super("invsee", "Opens the inventory of a player", "/invsee <player>", "", "nexus.command.invsee");
    }
    ...
    protected void execute(CommandSender sender, String label, String[] args) {
        if (args.length == 1) {
            ...
            sender.sendMessage(TextUtils.applyColor(Nexus.getInstance().localeReader.getPrefix() + Nexus.getInstance().localeReader.getString("invsee-usage")));
        }
    }
